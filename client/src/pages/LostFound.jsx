    ...
    >
      ...
      <div className="p-4 space-y-2">
        ...
        <a
          ...
          className="mt-1 w-full flex items-center justify-center gap-2 py-2.5 bg-primary/10 text-primary rounded-xl text-sm font-semibold hover:bg-primary hover:text-white transition-colors"
        >
          <Phone size={14} /> Contact {isLost ? 'Owner' : 'Finder'}
        </a>
      </div>
    </motion.div>
  );
...
function LostPetForm({ onClose, onSuccess }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-xs font-semibold text-text-light mb-1.5">Last Seen Location *</label>
...
export default function LostFound() {
  ...
  const mapPets = activePets.filter(p => p.latitude && p.longitude);
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          ...
          <div className="flex bg-neutral rounded-2xl p-1 gap-1">
            {[{ key: 'lost', label: '🔍 Lost Pets' }, { key: 'found', label: '💚 Found Pets' }].map(tab => (
              <button
                key={tab.key}
                id={`tab-${tab.key}`}
        ...
        {loading ? (
          ...
          <div className="h-[520px] rounded-3xl overflow-hidden border border-neutral shadow-sm">
            <MapContainer center={[17.385, 78.4867]} zoom={11} className="h-full w-full">
              <TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a>' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
              {mapPets.map((pet) => (
                <Marker key={pet._id} position={[pet.latitude, pet.longitude]}>
                  <Popup>
